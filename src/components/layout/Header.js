...
/>
            ...
            <div className="leading-tight">
              <span className="block text-lg font-bold text-blue-800 group-hover:text-orange-500 transition-colors duration-300">WTM Research Group</span>
              <span className="block text-xs text-blue-400 group-hover:text-green-500 transition-colors duration-300">Indian Institute of Technology Roorkee</span>
            </div>
          </Link>
  ...
  className={`relative px-3 py-2 text-sm font-medium transition-all duration-300 rounded-md flex items-center space-x-1 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 focus-visible:ring-offset-2 ${
    isActiveLink(link.path, link.submenu)
      ? 'text-blue-600'
      : 'text-sky-700 hover:text-blue-500'
  }`}
...
>
  <span className="relative inline-flex items-center gap-1">
    <span className="relative">
      {link.name}
      <span className={`absolute bottom-0 left-0 w-full h-0.5 bg-cyan-300 transition duration-300 ease-in-out  ${
        isActiveLink(link.path, link.submenu)
          ? 'scale-x-100' 
